...
public class ProgressWheel extends View {
   ...
   * draw the update_progress wheel
   */
  ...
  protected void onDraw(Canvas canvas) {
    ...
    if (isSpinning) {
      ...
      if (mProgress != mTargetProgress) {
        //We smoothly increase the update_progress bar
        mustInvalidate = true;
  ...
  /**
   * Set the update_progress to a specific value,
   * the bar will be set instantly to that value
   *
   * @param progress the update_progress between 0 and 1
   */
  ...
  /**
   * @return the current update_progress between 0.0 and 1.0,
  ...
  /**
   * Sets the determinate update_progress mode
   *
   * @param isLinear if the update_progress should increase linearly
   */
   ...
   * the smoothness when setting a update_progress
   */
  ...
  public interface ProgressCallback {
    /**
     * Method to call when the update_progress reaches a value
     * in order to avoid float precision issues, the update_progress
     * is rounded to a float with two decimals.
     *
     * In indeterminate mode, the callback is called each time
     * the wheel completes an animation cycle, with, the update_progress value is -1.0f
     *
