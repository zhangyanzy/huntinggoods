 ...
 */
package com.jinxun.hunting_goods.weight.pullrefresh.layout;
...
public class FlingLayout extends FrameLayout implements NestedScrollingChild, NestedScrollingParent {
  ...
  public int startMoveBy(float startY, float dy) {
    ...
    if (mPullView instanceof AbsListView) {
      ...
      valueAnimator = ValueAnimator.ofFloat(startY, startY + dy);
      valueAnimator.setDuration(time);
      valueAnimator.setInterpolator(new AccelerateDecelerateInterpolator());
      valueAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
        @Override
        public void onAnimationUpdate(ValueAnimator valueAnimator) {
          moveTo((float) valueAnimator.getAnimatedValue());
        }
      });
  ...
  public boolean dispatchTouchEvent(MotionEvent ev) {
    if (mPullView != null && !ViewCompat.isNestedScrollingEnabled(mPullView)) {
      ...
      if (mPullView instanceof AbsListView || mPullView instanceof ScrollView) {
        if (!mScroller.isFinished())
